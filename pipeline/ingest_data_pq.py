...
def load_data(url, table_name, db, month, year):
    df = pl.read_parquet(f'{url}_{year:04d}-{month:02d}.parquet').cast(pl_dtype)
    df.write_database(
...
def main(pg_user, pg_pass, pg_host, pg_port, pg_db, year, month, target_table):
    ...
    url = 'https://d37ci6vzurychx.cloudfront.net/trip-data/yellow_tripdata'

    load_data(url, target_table, db, month, year)
